...
class SRAccessBase {
  ...
  addAccess(access, opt) {
    this.AccessMap.set(access, opt);
  }
  ...
  checkAccess(userAccess, targetAccess, ctx) {
    // eslint-disable-next-line no-bitwise
    return [ (userAccess & targetAccess) === targetAccess, `访问被拒绝，需要${this.AccessMap.get(targetAccess).msg}权限` ];
  }

    ...
    for (const routeInfo of routeList) {
      const methodSwagger = {
        [`${routeInfo.method}`]: {
          ...
          tags: routeInfo.tags || [],
          parameters: routeInfo.method === EnumMethod.get ? { parameters: j2s(joi.object().keys(routeInfo.params)).swagger } :
            { requestBody: j2s(joi.object().keys(routeInfo.params)).swagger },
